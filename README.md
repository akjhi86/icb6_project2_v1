...
1. Python 환경에서 필요한 라이브러리를 설치합니다. (`uv` 또는 기본 `pip` 가상환경 활용 권장)
   ...
   ```
2. 작업 디렉터리를 프로젝트의 메인 경로(루트)로 위치시킨 뒤, 파이썬 분석 스크립트를 실행합니다.
   ```bash
   python src/eda_analysis.py
   ```
3. 코드가 실행 완료되면 `images/` 폴더에 시각화 그래프 10종이 자동으로 저장 및 갱신되며, 콘솔 창(혹은 로그)에 각종 기초통계표와 교차 데이터 결과들이 요약 표시됩니다. 해당 정보들은 `/docs/eda_report.md` 에 한 번에 자세히 문서화되어 있습니다.
